...
Strava is an excellent way to keep track of physical activity on an individual basis. To demonstrate how to use parallel iterations in R, the following tutorial will walk through splitting the data to display weekly trends based on activity type. 

We will begin by reading in a batch of data collected from one individual. We will tidy the data by filtering time and selecting start dates. This allows us to have a clean data set that is easy to work with.
...
There are two functions used to assist with creating a more friendly dataset. The first converts yards to meters to create more comprehensible numbers and the second is used to select only the most common activities. 
...
In order to display weekly trends, we will need to convert the standard date format into days of the week. A function is used to accomplish that. This conversion will come in handy when nesting data later on. 
